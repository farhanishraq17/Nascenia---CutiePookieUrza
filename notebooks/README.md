...
⚠️ The `train.ipynb` files are **specification templates** written before the run. The arms were actually launched as SLURM jobs calling [`scripts/pipeline/`](../scripts/pipeline/) — the exact invocations are in [`scripts/chpc_slurm/`](../scripts/chpc_slurm/). In particular `E05_train_to_convergence/train.ipynb` has the wrong data dir and sequence lengths for the champion; use `scripts/chpc_slurm/nasc-E05-english_draft.sbatch`.
