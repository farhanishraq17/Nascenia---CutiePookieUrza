...
⚠️ Their task is summarisation/extraction from multi-turn dialogue; ours is register transfer on
...
| **`google/gemma-2-2b-it`** | 2.6B | Best zero-shot generation in the paper, best fine-tuned summarisation. ⚠️ Fits the cap **alone** — no room to ensemble |
| `Qwen/Qwen2.5-1.5B-Instruct` | 1.54B | The ≤2.5B hedge CLAUDE.md named and never ran. ⚠️ Qwen2.5's language list does **not** feature Bengali — expect it below Qwen3 |
...
⚠️ **Qwen3: set `enable_thinking=False`.** Register transfer is not a reasoning task; thinking
...
| Gemma wins but only just | 2.6B beats 248M by a little | ⚠️ Weigh carefully — **Gemma leaves no ensemble headroom**, BanglaT5 leaves room for 10 |
