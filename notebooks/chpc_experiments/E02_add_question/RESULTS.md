...
>    exhausted at 8/8.
>
> **⚠️ `../_slurm/analysis/scoreboard.md` ranks E02/conv12k's 0.7953 in the same column as those
...
⚠️ This arm was decoded twice: once before the `04_decode.py --max-source-len` fix (which fed it
...
⚠️ **Honest bound on what is missing.** The in-training curve is flat over the last third:
...
⚠️ Quote the hash from `run.json`, not from `dev.json`. The `checkpoint_hashes` field inside the
