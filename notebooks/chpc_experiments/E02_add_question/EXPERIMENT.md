...
**Tier 1 — INFORMATION · Priority: 🥈 HIGH**

## The question

> Does the **patient question** add information? (The incumbent never sees it.)

## Why it matters

**The 0.85030 model does not read the patient's question at all** — it only sees our Bengali
draft. That is a genuine blind spot nobody has tested.

The question could disambiguate: when the draft is vague or our translation garbled a term, the
question says what the patient actually asked. It is also the *only* field the organizers guarantee
at test time, so anything learned from it is robust even if the alignment route is ever closed off.

Cheap, and completely independent of E01 — run them in parallel.
...
python ../code/02_train_t5.py \
  ...
  --seed 11 --epochs 2.516 --run-name E02_banglat5_question_draft
```

Then decode and score:

```bash
python ../code/04_decode.py --ckpt runs/E02_banglat5_question_draft/best --data-dir ../data/question_draft \
...
**The incumbent: Token F1 **0.7724** · ROUGE-L **0.7324** · public LB **0.85030**.** Also compare against E01 — if both gain, run E03.

## How to read the result

| Result | Meaning |
|---|---|
| **> 0.7724** | The question carries signal the draft lacks → **E03 (all three fields) becomes the priority** |
| ≈ 0.7724 | The draft already contains everything the question would tell you |
| < 0.7724 | The question is noise here — the answer is fully determined by the draft |
