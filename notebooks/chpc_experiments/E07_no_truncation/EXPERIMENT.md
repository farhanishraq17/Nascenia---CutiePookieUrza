...
**Tier 2 — TRAINING REGIME · Priority: MEDIUM**

## The question

> How much is **truncation** costing on the longest inputs?

## Why it matters

Measured on the `all_inputs` dataset, sources average 290 words and reach p95 455. Every T4
configuration truncated something, and **truncated source is information the model can never
recover** — a permanently invisible tail.

This run removes the cap entirely (1280/768 covers p99 on both sides) and is only possible with
large VRAM. It is the control that tells you whether any weak Tier-1 result was the *hypothesis*
failing or merely the *context window*.
...
| Sequence | `--max-source-len 1280 --max-target-len 768` |
| Batch | `--batch-size 8 --grad-accum 8` (effective 64) |
...
python ../code/02_train_t5.py \
  ...
  --seed 11 --epochs 2.516 --run-name E07_no_truncation
```

Then decode and score:

```bash
python ../code/04_decode.py --ckpt runs/E07_no_truncation/best --data-dir ../data/all_inputs \
...
**E03 (all_inputs at 1024/512).** The delta is the cost of truncation.
