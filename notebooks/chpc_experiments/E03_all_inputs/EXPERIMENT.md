# E03 — BanglaT5 + all three inputs

**Tier 1 — INFORMATION · Priority: HIGH (after E01/E02)**

## The question

> Does **everything available** beat any single addition?

## Why it matters

Question + English + draft — every signal that exists for a test row. The maximum-information
configuration, and the ceiling for the input-side lever.

Sources average 290 words (p95 455), so this needs the longest context of any experiment. It is also
the most likely to be *hurt* by truncation, which is exactly why it belongs on a big GPU rather than
a T4 — verify no truncation before believing a weak result.
...
| Sequence | `--max-source-len 1024 --max-target-len 512` |
...
python ../code/02_train_t5.py \
  ...
  --seed 11 --epochs 2.516 --run-name E03_banglat5_all_inputs
```

Then decode and score:

```bash
python ../code/04_decode.py --ckpt runs/E03_banglat5_all_inputs/best --data-dir ../data/all_inputs \
...
**E01 and E02 individually**, and the incumbent 0.7724. Gains may not be additive.

## How to read the result

| Result | Meaning |
|---|---|
| > both E01 and E02 | The signals are complementary — this is the input configuration to ship |
| ≈ max(E01, E02) | One field dominates; the others are redundant. Prefer the **shorter** input — it trains faster |
| < max(E01, E02) | Too much context is diluting attention. Trust the simpler winner |
