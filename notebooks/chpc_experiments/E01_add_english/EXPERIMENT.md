...
**Tier 1 — INFORMATION · Priority: HIGHEST**

## The question

> Does the **English source** add information the Bengali draft lost?

## Why it matters

The incumbent reads our Bengali draft and rewrites it in the competition's register — so it
must *invert our translation and re-apply theirs*, two hops, with our draft's noise in its input.
The English original is the **common ancestor of both translations**.

This matters because capacity is measurably **not** the bottleneck: two BanglaT5 seeds landed at
0.7724 and 0.7723 — a gap of 0.0001. When two independent runs agree that closely, only more
*information* can help, not a bigger model.

**Run this first.** It changes exactly ONE variable from a model with a real leaderboard score.
...
python ../code/02_train_t5.py \
  ...
  --seed 11 --epochs 2.516 --run-name E01_banglat5_english_draft
```

Then decode and score:

```bash
python ../code/04_decode.py --ckpt runs/E01_banglat5_english_draft/best --data-dir ../data/english_draft \
...
**The incumbent: Token F1 **0.7724** · ROUGE-L **0.7324** · public LB **0.85030**** — same model, same data, draft-only input.

## How to read the result

| Result | Meaning | Next |
|---|---|---|
| **> 0.7724** | English adds information. The cheapest real win available. | Ship it; then E03/E05 |
| ≈ 0.7724 | The draft already carries what English would add | Drop the English line; go to E02 |
| < 0.7724 | English is *distracting* — longer input, no gain | Check truncation first, then E04 |
