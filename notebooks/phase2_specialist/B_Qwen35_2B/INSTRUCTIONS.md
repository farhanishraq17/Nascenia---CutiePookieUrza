...
| `OPTIM` | `adamw_torch` | AdamW keeps fp32 m+v ≈ **12 bytes/param** — for 2B that is ~24 GB of optimizer state alone, before weights, gradients or activations. On a 24 GB card **switch to `adafactor`**, which factors the second moment and fits. This is not an exotic choice; it is what the champion trained with |
...
There is a specific reason a decoder might diverge from mT5 here: all three extras are much
...
All three extras open with `হেলো` **0.0%** of the time against the competition's **76.4%** — the
