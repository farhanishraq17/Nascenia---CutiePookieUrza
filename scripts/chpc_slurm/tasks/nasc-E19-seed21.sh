...
$PY ../code/04_decode.py --ckpt seed21/best --data-dir ../data/english_draft \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record seed21/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt seed21/best --data-dir ../data/english_draft \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out seed21/submission.csv --record seed21/test.json
