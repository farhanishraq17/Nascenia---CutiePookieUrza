...
$PY ../code/04_decode.py --ckpt len40/best --data-dir ../data/english_draft_len40 \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record len40/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt len40/best --data-dir ../data/english_draft_len40 \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out len40/submission.csv --record len40/test.json
