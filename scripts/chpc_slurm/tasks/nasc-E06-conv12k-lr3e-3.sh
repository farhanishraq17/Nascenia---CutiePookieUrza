...
cd /scratch/general/nfs1/u1592009/Nascenia_Datathon/fine_tune_project/E06_lr_retune
...
$PY ../code/02_train_t5.py \
  ...
  --out-dir . --run-name conv12k-lr3e-3 \
  --seed 11 \
  --lr 0.003 --warmup 200 --optim adafactor \
...
$PY ../code/04_decode.py --ckpt conv12k-lr3e-3/best --data-dir ../data/english_draft \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record conv12k-lr3e-3/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt conv12k-lr3e-3/best --data-dir ../data/english_draft \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out conv12k-lr3e-3/submission.csv --record conv12k-lr3e-3/test.json
