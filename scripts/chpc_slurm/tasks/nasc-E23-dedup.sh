...
$PY ../code/04_decode.py --ckpt dedup/best --data-dir ../data/english_draft_dedup \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record dedup/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt dedup/best --data-dir ../data/english_draft_dedup \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out dedup/submission.csv --record dedup/test.json
