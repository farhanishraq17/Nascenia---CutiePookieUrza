...
$PY ../code/04_decode.py --ckpt clean/best --data-dir ../data/english_draft_clean \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record clean/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt clean/best --data-dir ../data/english_draft_clean \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out clean/submission.csv --record clean/test.json
