...
$PY ../code/02_train_t5.py \
  --data-dir ../data/english_draft_len60 \
  --model csebuetnlp/banglat5 \
  --out-dir . --run-name len60 \
  --seed 11 \
...
$PY ../code/04_decode.py --ckpt len60/best --data-dir ../data/english_draft_len60 \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record len60/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt len60/best --data-dir ../data/english_draft_len60 \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out len60/submission.csv --record len60/test.json
