...
$PY ../code/02_train_t5.py \
  --data-dir ../data/english_draft_rag \
...
$PY ../code/04_decode.py --ckpt B/best --data-dir ../data/english_draft_rag \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out B/submission.csv --record B/test.json
