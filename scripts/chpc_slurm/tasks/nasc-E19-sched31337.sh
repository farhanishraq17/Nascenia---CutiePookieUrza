...
$PY ../code/04_decode.py --ckpt sched31337/best --data-dir ../data/english_draft \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record sched31337/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt sched31337/best --data-dir ../data/english_draft \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out sched31337/submission.csv --record sched31337/test.json
