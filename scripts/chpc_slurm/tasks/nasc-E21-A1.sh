...
cd /scratch/general/nfs1/u1592009/Nascenia_Datathon/fine_tune_project/E21_synthetic_pairs

# ---- train ------------------------------------------------------------------
# --resume makes a requeued job pick up the last checkpoint instead of restarting.
$PY ../code/02_train_t5.py \
  --data-dir ../data/english_draft_a1 \
...
$PY ../code/04_decode.py --ckpt A1/best --data-dir ../data/english_draft_a1 \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out A1/submission.csv --record A1/test.json
