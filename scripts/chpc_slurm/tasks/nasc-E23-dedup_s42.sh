...
$PY ../code/04_decode.py --ckpt dedup_s42/best --data-dir ../data/english_draft_dedup \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record dedup_s42/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt dedup_s42/best --data-dir ../data/english_draft_dedup \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out dedup_s42/submission.csv --record dedup_s42/test.json
