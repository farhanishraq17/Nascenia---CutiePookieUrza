...
$PY ../code/02_train_t5.py \
  --data-dir ../data/english_draft_a2 \
...
$PY ../code/04_decode.py --ckpt A2/best --data-dir ../data/english_draft_a2 \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out A2/submission.csv --record A2/test.json
