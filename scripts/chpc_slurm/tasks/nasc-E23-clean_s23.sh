...
$PY ../code/02_train_t5.py \
  --data-dir ../data/english_draft_clean \
  --model csebuetnlp/banglat5 \
  --out-dir . --run-name clean_s23 \
  --seed 23 \
...
$PY ../code/04_decode.py --ckpt clean_s23/best --data-dir ../data/english_draft_clean \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record clean_s23/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt clean_s23/best --data-dir ../data/english_draft_clean \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out clean_s23/submission.csv --record clean_s23/test.json
