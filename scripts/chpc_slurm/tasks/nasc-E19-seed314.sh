...
$PY ../code/04_decode.py --ckpt seed314/best --data-dir ../data/english_draft \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record seed314/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt seed314/best --data-dir ../data/english_draft \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out seed314/submission.csv --record seed314/test.json
