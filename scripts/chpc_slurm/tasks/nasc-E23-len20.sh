...
$PY ../code/02_train_t5.py \
  --data-dir ../data/english_draft_len20 \
  --model csebuetnlp/banglat5 \
  --out-dir . --run-name len20 \
  --seed 11 \
...
$PY ../code/04_decode.py --ckpt len20/best --data-dir ../data/english_draft_len20 \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record len20/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt len20/best --data-dir ../data/english_draft_len20 \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out len20/submission.csv --record len20/test.json
