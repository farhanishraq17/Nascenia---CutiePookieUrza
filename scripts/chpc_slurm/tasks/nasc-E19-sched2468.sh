...
$PY ../code/04_decode.py --ckpt sched2468/best --data-dir ../data/english_draft \
  --split dev --limit 300 --mode beam --num-beams 4 \
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --record sched2468/dev.json
echo "decode exit=$?"
...
$PY ../code/04_decode.py --ckpt sched2468/best --data-dir ../data/english_draft \
  ...
  --min-new-tokens 80 --max-new-tokens 320 --length-penalty 1.0 \
  --max-source-len 768 \
  --no-bertscore --out sched2468/submission.csv --record sched2468/test.json
