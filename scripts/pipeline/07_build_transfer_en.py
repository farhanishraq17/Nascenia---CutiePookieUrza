...
def main(argv=None) -> int:
    ...
    for split in ("train", "dev", "test"):
        ...
        src = ([str(x) for x in en.loc[have["id"]]] if a.english_only else
               [TEMPLATE.format(en=e, bn=b) for e, b in
                zip(en.loc[have["id"]], bn.loc[have["id"]])])
    ...
    print(f"\nwritten to {out}   mode = {'ENGLISH ONLY' if a.english_only else 'english + bengali draft'}")
    return 0
