...
class Decoder:
    def __init__(self, ckpts: list[str], device: str | None = None, fp16: bool = False):
        ...
        for c in ckpts:
            ...
            with torch.no_grad():
                ...
                # Same reason as in run(): IndicBART's Albert tokenizer emits
                # token_type_ids that MBart.forward() rejects. Keep only what every
                # seq2seq forward() accepts.
    ...
    def generate(self, texts, model_idx=0, *, mode="beam", n=1, num_beams=4,
        ...
        for i in range(0, len(texts), batch_size):
            ...
            # IndicBART pairs an Albert tokenizer with an MBart model: the tokenizer
            # emits token_type_ids, MBart.forward() does not accept them, and generate()
            # dies with a bare TypeError AFTER training has already finished. Drop any
            # key the model cannot take rather than special-casing one architecture.
...
def main(argv=None) -> int:
    ...
    # 🔴 Default None, NOT 384. It used to default to 384 while E01 trains at 768, E03 at
    # 1024 and E07 at 1280 — so decoding silently truncated inputs the model was trained to
    # read, and every one of those arms was under-scored by its own eval. Nothing crashes,
    # nothing warns: the CSV is well-formed, just built from a third of the input. When not
    # given, the cap is read from the run.json beside the checkpoint.
    ...
    # Decode at the cap the model was TRAINED at. run.json sits one level above best/.
    max_src = a.max_source_len
    if max_src is None:
        for c in a.ckpt:
            rj = Path(c).parent / "run.json"
            if rj.is_file():
                trained = json.loads(rj.read_text(encoding="utf-8")).get("max_source_len")
                if trained:
                    max_src = max(max_src or 0, int(trained))
        if max_src:
            print(f"max_source_len {max_src} (from run.json — the cap it trained at)")
        else:
            max_src = 384
            print("⚠️  no run.json beside the checkpoint; falling back to "
                  "max_source_len 384. If this model trained on longer inputs, its score "
                  "here is an underestimate — pass --max-source-len explicitly.")
    ...
    gen_kw = dict(num_beams=a.num_beams, temperature=a.temperature, top_p=a.top_p,
                  ...
                  seed=a.seed, max_source_len=max_src)
    ...
    # ---------------- register read-out ----------------
    # README calls this "the sharpest single diagnostic available": near the DRAFT's rates
    # means the model copied its input, near the REFERENCES' means it learned the
    # conversion. Cheap to compute, and required for every run, so it lives in the record
    # rather than in whatever ad-hoc script happens to be around at reporting time.
    n = max(len(preds), 1)
    record["register"] = {
        "helo_opener_pct": round(100.0 * sum(p.lstrip().startswith("হেলো")
                                             for p in preds) / n, 2),
        "nasenia_pct": round(100.0 * sum("নাসেনিয়া" in p for p in preds) / n, 2),
        "refs_helo_opener_pct": 76.4, "refs_nasenia_pct": 50.0,
        "draft_helo_opener_pct": 0.06, "draft_nasenia_pct": 0.0,
    }
    print(f"\n  register: হেলো opener {record['register']['helo_opener_pct']:.1f}% "
          f"(refs 76.4, draft 0.06) · নাসেনিয়া "
          f"{record['register']['nasenia_pct']:.1f}% (refs 50.0, draft 0.0)")
    # Predictions themselves: E14 and E19 both gate on ROW-LEVEL DISAGREEMENT between
    # members, which cannot be recovered from summary metrics after the fact.
